...
- [ ] フェーズ2B完了：全ステートメントの再実装
  ...
  - [ ] 2B.3.5: 単項マイナス演算子と負の数リテラル ← **次のステップ**
  - [⚠️] 2B.4: GOTO/GOSUB/RETURN/HALTステートメントの解析（要修正：ラベルのみ許容）
  - [ ] 2B.5: FORループ・NEXTの解析
  - [ ] 2B.6: PEEK/POKEの解析
  - [ ] 2B.7: 複数ステートメント行の統合テスト
- [ ] フェーズ3開始：インタプリタ実装

## 現在の進捗

**完了:** TDDサイクル 2B.3（IFステートメント再設計）  
**修正中:** TDDサイクル 2B.4（GOTO/GOSUB/RETURN/HALT - ラベルのみ許容に修正）  
**次のステップ:** TDDサイクル 2B.3.5（単項マイナス演算子と負の数リテラル）

### 重要な修正事項

**TDDサイクル 2B.4の仕様違反を発見:**
- 初期実装では誤ってGOTO/GOSUBが数値・変数・式を許容していた
- WorkerScript仕様では **ラベル（`^LABEL`形式）のみ** を受け入れる
- VTL2では行番号を使用していたが、WorkerScriptではラベルベース
- AST定義と実装を修正してから次のサイクルに進む

### 2B.3.5の重要性

FOR/NEXTループ（2B.5）で負のステップ値（`I=100,1,-1`）が必要になるため、
その前に単項マイナス演算子のサポートを完了する必要があります。
